...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        current_frame = Frame(procedure.env) # takes the lambda function's env
        for vals in procedure.formals:
            current_frame.bindings[vals] = args.first # values are bounded to the frame's formal parameters 
            args = args.second
        return scheme_eval(procedure.body, current_frame)
    elif isinstance(procedure, MuProcedure):
        for vals in procedure.formals:
            env.bindings[vals] = args.first # values are bounded to parent's formal parameters
            args = args.second
...
class Frame(object):
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) < len(vals):
            raise Error('too few arguments are given')
        if len(formals) > len(vals):
            raise Error('too many arguments are given')
        while len(formals) > 0:
            frame.bindings[formals[0]] = vals[0] # binds values to formal parameters
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first): # checks that target.first is a symbol
            Frame.define(env, target.first, do_lambda_form(Pair(target.second, vals.second), env)) # assigns lambda to target.first
            return target.first
...
def do_let_form(vals, env):
    ...
    for symbol, value in vals[0]: # creates list of symbol and values 
        names = Pair(symbol, names)
        values = Pair(scheme_eval(value, env), values)
    new_env = env.make_call_frame(names, values) # assigns values to symbols in given env
...
def do_and_form(vals, env):
    ...
    val = True
    for val in vals:
        if not scheme_true(scheme_eval(val, env)):
            return False
    return val
...
def do_or_form(vals, env):
    ...
    for val in vals:
        if scheme_true(scheme_eval(val, env)):
            return quote(scheme_eval(val, env))
    return val
    ...
    for i, clause in enumerate(vals):
        ...
        if scheme_true(test):
            if len(clause.second) > 1:
                return scheme_eval(Pair('begin', clause.second), env) # handles multiple return condition
            if len(clause) == 1:
                if clause.first:
                    return clause.first
            return clause[1]
...
def check_formals(formals):
    ...
    for formal in formals:
        if not scheme_symbolp(formal) or formal in seen:
            raise SchemeError('invalid list of formal parameters')
        seen += [formal]
