...
;;; testing apply_primitive and lookup 
...
(and 'vincent 'lawrence 'harrison)
; expect harrison

(and 20 (= 1 2) 'hi)
; expect False

(or 'creampuff (/ 1 0) 9000)
; expect creampuff
...
(define (list-partitions total max-pieces max-value)
  (all-partitions total max-pieces max-value nil))

(define (all-partitions total max-pieces max-value answer)
  (cond ((= total 0) (cons answer nil))
      ((< total 0) nil)
      ((= max-value 0) nil)
      ((= max-pieces 0) nil)
    (else
      (append (all-partitions (- total max-value) (- max-pieces 1) max-value (cons max-value answer))
        (all-partitions total max-pieces (- max-value 1) answer)))))
...
(define (tree-sum-list tree)
  ...
  (map-two + (entry tree) (map-one tree-sum-list (children tree)))))

(define (append s1 s2)
  (if (null? s1) s2
    (cons (car s1) (append (cdr s1) s2))))

(define (map-two fn value lst)
  (if (null? lst) nil
    (cons (fn value (car lst)) (map-two fn value (cdr lst)))))

(define (map-one fn lst)
  (if (null? lst) nil
    (append (fn (car lst)) (map-one fn (cdr lst)))))
