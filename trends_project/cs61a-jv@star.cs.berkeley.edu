...
def analyze_tweet_sentiment(tweet):
    ...
    False
    """
    word_lst, total, word_count = tweet_words(tweet), 0, 0
    for elem in word_lst:
        if has_sentiment(get_word_sentiment(elem)): # if a tweet has a sentiment value, add it to the total
            total += sentiment_value(get_word_sentiment(elem))
            word_count += 1
    if word_count == 0:
        return make_sentiment(None)
    return make_sentiment(total / word_count) # average of the sentiment value over the number of tweets 
...
def find_centroid(polygon):
    ...
    (1.0, 2.0, 0.0)
    """
    area, x_center, y_center = 0, 0, 0
    for i in range(len(polygon)-1):
        area += (latitude(polygon[i]) * longitude(polygon[i+1])) - (latitude(polygon[i+1]) * longitude(polygon[i]))
        x_center += (latitude(polygon[i]) + latitude(polygon[i+1])) * (latitude(polygon[i]) * longitude(polygon[i+1]) - latitude(polygon[i+1]) * longitude(polygon[i]))
        y_center += (longitude(polygon[i]) + longitude(polygon[i+1])) * (latitude(polygon[i]) * longitude(polygon[i+1]) - latitude(polygon[i+1]) * longitude(polygon[i]))
    final_area = area/2
    if final_area == 0:
        return (latitude(polygon[0]), longitude(polygon[0]), final_area)
    x_final_center = x_center/(6 * final_area)
    y_final_center = y_center/(6 * final_area)
    return (x_final_center, y_final_center, abs(final_area)) # take absolute value of area after calculations
...
def find_state_center(polygons):
    ...
    -156.21763
    """
    area_sum, x_center_num, y_center_num = 0, 0, 0
    for elem in polygons:
        x_center_num += (find_centroid(elem)[0] * find_centroid(elem)[2])
        y_center_num += (find_centroid(elem)[1] * find_centroid(elem)[2])
        area_sum += find_centroid(elem)[2]
    total_x_center = x_center_num/area_sum
    total_y_center = y_center_num/area_sum
    return make_position(total_x_center, total_y_center)
...
def find_closest_state(tweet, state_centers):
    ...
    """
    place = 'CA'
    closest = geo_distance(state_centers['CA'],tweet_location(tweet)) # CA is default (California pride!)
    for k, v in state_centers.items():
        distance = geo_distance(v, tweet_location(tweet))
        if distance < closest: 
            place = k # place contains the state where the tweet is closest to
            closest = distance 
    return place
...
def group_tweets_by_state(tweets):
    ...
    us_centers = {n: find_state_center(s) for n, s in us_states.items()}
    for elem in tweets:
        if find_closest_state(elem, us_centers) in tweets_by_state:
...
def most_talkative_states(term):
    ...
    grouped_tweets, lst = group_tweets_by_state(tweets), []
    for k, v in grouped_tweets.items(): # creates a list of tuples of states and the number of tweets containing term 
        lst += [(k, len(v))]
    def helper(lst, largest, count, winners): #recurses until 5 winners are selected 
        if count == 5:
            return winners
        ...
        winners += [place]
        lst.remove(place)
        return helper(lst, 0, count + 1, winners)
    return helper(sorted(lst), 0, 0, [])
...
def average_sentiments(tweets_by_state):
    ...
    """
    averaged_state_sentiments, sentiment_score = {}, 0
    for k, v in tweets_by_state.items():
        sentiment_score, num_tweets = 0, 0
        for elem in v:
            if analyze_tweet_sentiment(elem) != None:
                sentiment_score += analyze_tweet_sentiment(elem)
                num_tweets += 1
        if num_tweets != 0:
            averaged_state_sentiments[k] = sentiment_score/num_tweets
...
def group_tweets_by_hour(tweets):
    ...
    for i in range(24):
        tweets_by_hour[i] = []
        for elem in tweets:
            if tweet_time(elem).hour == i:
                tweets_by_hour[i].append(elem)
