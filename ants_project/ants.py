...
Name: Chung-Hao Yu, Andrew Wong
Login: cs61a-jv, cs61a-br
TA: Mark Miyashita, Rohan Chitnis 
Section: 106, 101
"""
...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant != None:
                if self.ant.can_contain(insect): # condition where bodyguard is already placed
                    self.ant.contain_ant(insect)
                    return
                elif insect.can_contain(self.ant): # condition where bodyguard is placed onto ant
                    current_place = self.ant
                    self.remove_insect(self.ant)
                    self.add_insect(insect)
                    insect.contain_ant(current_place)
                    return
    ...
    def remove_insect(self, insect):
        ...
        if not insect.is_ant():
            ...
            if insect.container and insect.ant != None: # condition where bodyguard is containing another ant 
                self.ant = insect.ant
            else:
                self.ant = None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        check_place, throw_range= self.place, 0
        while check_place != hive and check_place != None:    
            if random_or_none(check_place.bees) != None and self.min_range <= throw_range <= self.max_range:
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        if self.place.bees:
            for bees in self.place.bees[:]: # damages all bees in its place
                bees.reduce_armor(1)
...
class BodyguardAnt(Ant):
    ...
    def contain_ant(self, ant):
        self.ant = ant
        self.ant.place = self.place # stores the current place into the stored ant's place

    def action(self, colony):
        if self.ant != None:
            self.ant.action(colony) # bodyguard takes the action of the ant it is containing 
...
class PrincessAnt(Ant):
    ...
    def action(self, colony):
        ...
        if not self.action_performed:
            ...
            if type(self.place.ant) == BodyguardAnt: # condition if princess ant is placed in bodyguard ant 
                self.place.ant.armor += 1
            while current_place != Hive and current_place != None: # increases all ants' armor to the right of the princess
                if current_place.ant != None:
                    current_place.ant.armor += 1
                current_place = current_place.exit
            current_place = self.place.entrance
            while current_place != colony.queen and current_place != None: # increases all ants' armor to the left of the princess
                if current_place.ant != None:
                    current_place.ant.armor += 1
